...
class Backtest:
    ...
    def calculate(self, date, symbol1_price, symbol2_price, index):
        ...
        # ----------ratio calculation  --------------
        division_result = round(
            self.df[self.symbol_1][index]/self.df[self.symbol_2][index], 2)
        ...
        if index+1 < self.hedge_ratio_count_limit:
            ...
            if self.df_runtime['rolling_net_open'][index-1] == 0 and ratio == mean(self.df_runtime["hedge_ratio_calc"][index+1-self.hedge_ratio_count_limit: index+1]):
                ratio_trade = mean(
                    self.df_runtime["hedge_ratio_calc"][index+1-self.hedge_ratio_count_limit: index+1])
            else:
                ratio_trade = self.df_runtime["hedge_ratio_trade"][index-1]
        if index != 0:
            hedge_ratio_count = self.df_runtime["hedge_ratio_change_count"][index-1] + \
                1 if self.df_runtime["hedge_ratio_trade"][index -
                                                          1] == ratio_trade else 1

        # ----------if reinvest enabled ------------
        if index > 0:
            self.initial_investment = initial_investment_plus_strategy_net_cum_pnl if self.reinvest else self.df_runtime[
                "initial_investment"][index-1]
        ...
        if index == 0:
            ...
            if ratio_2d < 1:
                ...
                if ratio_2d_value_band == 0.5 and self.df_runtime['rolling_net_open'][index-1] == 0:
                    symbol_1_qty_multipier = 2
                else:
                    symbol_1_qty_multipier = self.df_runtime[self.qty_multiplier_1_string][index-1]
                symbol_2_qty_multipier = symbol_1_qty_multipier * ratio_trade

        # ----------Position Size multiplier Calculation -------------------------------
        # print(f'----------{self.symbol_1} {self.symbol_2} ')
        # print(f' {self.initial_investment} / ({symbol_1_qty_multipier} * {symbol1_price} + {symbol_2_qty_multipier} * {symbol2_price})/{self.max_position}')
        position_size_calculation = self.initial_investment / \
            (symbol_1_qty_multipier * symbol1_price +
             symbol_2_qty_multipier * symbol2_price)/self.max_position
        # print(position_size_calculation)
        ...
        if index == 0:
            ...
            position_size_trade = position_size_calculation if self.df_runtime['rolling_net_open'][
                index-1] == 0 else self.df_runtime['position_size_trade'][index-1]

        # ----------demat requirement -------------------
        stock_1_demat_req = symbol_1_qty_multipier * \
            position_size_trade * self.max_position
        stock_2_demat_req = symbol_2_qty_multipier * \
            position_size_trade * self.max_position
        ...
        # ----------spread -----------------------
        spread = round((symbol1_price * symbol_1_qty_multipier) -
                       (symbol2_price * symbol_2_qty_multipier), 2)

        # ----------moving average & standard deviation -----------
        ma = 0
        sd = 0
        if index >= self.moving_average-1:
            spread_list = list(self.df_runtime['spread'][(
                index+1-self.moving_average):index + 1])
            spread_list.append(spread)
            ma = round(mean(spread_list), 2)

        if index >= self.standard_deviation-1:
            spread_list = list(self.df_runtime['spread'][(
                index+1-self.standard_deviation):index+1])
            spread_list.append(spread)
        ...
        if index >= self.days_to_wait-1:
            ...
            if spread < lower_bound:
                ...
                if self.df_runtime["rolling_net_open"][index-1] < self.max_position and hedge_ratio_count >= self.moving_average and hedge_ratio_count >= self.standard_deviation:
            ...
            if spread > upper_bound:
                ...
                if (self.df_runtime["rolling_net_open"][index-1] * -1) < self.max_position and hedge_ratio_count >= self.moving_average and hedge_ratio_count >= self.standard_deviation:
        ...
        if index == 0:
            ...
            rolling_net_open = net_open + \
                self.df_runtime['rolling_net_open'][index-1]

        # ----------strategy day gross pnl  --------------------
        strategy_day_gross_pnl = (
            sell_signal_accepted * 1) - (buy_signal_accepted * 1) + net_open*spread
        strategy_gross_cum_pnl = copy.deepcopy(strategy_day_gross_pnl)
        if index != 0:
            prev_rolling_net_open = self.df_runtime["rolling_net_open"][index-1]
            strategy_day_gross_pnl = round(((strategy_day_gross_pnl)+prev_rolling_net_open * self.df_runtime["spread"][index-1] * -1
                                            + prev_rolling_net_open * spread) * position_size_trade, 2)
            strategy_gross_cum_pnl = strategy_day_gross_pnl + \
                self.df_runtime["strategy_gross_cum_pnl"][index-1]

        # ----------turnover : ask question about round up------------
        turnover = abs(net_open) * (stock_1_trade_exec_qty *
                                    symbol1_price + stock_2_trade_exec_qty * symbol2_price)
        ...
        if index != 0:
            cum_transaction_costs = round(
                cum_transaction_costs + self.df_runtime["cum_transaction_costs"][index-1], 2)

        # ----------strategy net cum pnl - transaction costs are -ve hence adading it ----------------
        strategy_net_cum_pnl = round(
            strategy_gross_cum_pnl+cum_transaction_costs, 2)

        # ----------net_withdraw_reinvestable_calculation -----------
        net_withdraw_reinvestabe_calculation = 0
        if index >= 2:
            net_withdraw_reinvestabe_calculation = strategy_net_cum_pnl if rolling_net_open == 0 else self.df_runtime[
                "net_withdraw_reinvestabe_calculation"][index-1]
            net_withdraw_reinvestabe_calculation = round(
                net_withdraw_reinvestabe_calculation, 2)

        # -----------Net P&L (Withdraw / Reinvest able) Trade by Trade P&L Closed Trades  ---------------------
        net_pnl_tradebytrade_closedtrades = 0
        if index >= 2:
            net_pnl_tradebytrade_closedtrades = (
                net_withdraw_reinvestabe_calculation-self.df_runtime["net_pnl_tradebytrade_closedtrades"][index-1]) if rolling_net_open == 0 else 0
            net_pnl_tradebytrade_closedtrades = round(
                net_pnl_tradebytrade_closedtrades, 2)

        # -----------initial_investment_plus_strategy_net_cum_pnl ----------------
        initial_investment_plus_strategy_net_cum_pnl = round(
            strategy_net_cum_pnl + self.initial_investment, 2)

        # -----------daily % change in net pnl : about division --------------------------
        daily_percent_change_net_pnl = 0
        if index > 0:
            daily_percent_change_net_pnl = initial_investment_plus_strategy_net_cum_pnl / \
                self.df_runtime["initial_investment_plus_strategy_net_cum_pnl"][index-1]-1
            daily_percent_change_net_pnl = round(
                daily_percent_change_net_pnl*100, 2)

        # -----------initial investment plus strategy net cum pnl drawdown -----------------
        initial_investment_plus_cum_pnl_drawdown = 0
        if index > 0:
            initial_investment_plus_cum_pnl_drawdown = initial_investment_plus_strategy_net_cum_pnl / \
                max(list(self.df_runtime["initial_investment_plus_strategy_net_cum_pnl"][0:index])+[
                    initial_investment_plus_strategy_net_cum_pnl])
            initial_investment_plus_cum_pnl_drawdown = round(
                (initial_investment_plus_cum_pnl_drawdown - 1) * 100, 2)
        ...
        if index >= 60:
            ...
            # round up by 1
            total_strategy_return = initial_investment_plus_strategy_net_cum_pnl / \
                self.df_runtime["initial_investment_plus_strategy_net_cum_pnl"][index-60]-1
            total_strategy_return = round((total_strategy_return)*100, 1)

            # round up by 1, there is some minor difference because of ^252
            CAGR_strategy_return = initial_investment_plus_strategy_net_cum_pnl / \
                self.df_runtime["initial_investment_plus_strategy_net_cum_pnl"][index-60]
            ...
            volatility_annualized = stdev(list(
                self.df_runtime["daily_percent_change_net_pnl"][index-60:index+1]) + [daily_percent_change_net_pnl])
            volatility_annualized = round(
                volatility_annualized*math.sqrt(252), 1)

            downside_volatility = [x for x in list(
                self.df_runtime["daily_percent_change_net_pnl"][index-60:index+1]) + [daily_percent_change_net_pnl] if x < 0]
            downside_volatility = round(
                stdev(downside_volatility)*math.sqrt(252), 1)

            max_drawdown = round(abs(min(list(self.df_runtime["initial_investment_plus_cum_pnl_drawdown"][index-60:index+1])+[
                                 initial_investment_plus_cum_pnl_drawdown])), 1)

            sharpe_ratio_net_pnl = round(
                (CAGR_strategy_return - volatility_annualized) / 5, 1)
        ...
        if index > 0:
            ...
            # leg1 calculation
            diff = stock_1_demat_req - \
                self.df_runtime["stock_1_demat_req"][index-1]
            bnh_buy_symbol1_qty = diff if diff > 0 else 0
            bnh_sell_symbol1_qty = abs(diff) if diff < 0 else 0
            bnh_net_open_symbol1 = bnh_buy_symbol1_qty - bnh_sell_symbol1_qty
            bnh_rolling_net_open_symbol1 = bnh_net_open_symbol1 + \
                self.df_runtime["bnh_rolling_net_open_symbol1"][index-1]
            bnh_day_gross_pnl_symbol1 = bnh_sell_symbol1_qty * symbol1_price - bnh_buy_symbol1_qty * symbol1_price + \
                bnh_net_open_symbol1 * symbol1_price + self.df_runtime["bnh_rolling_net_open_symbol1"][index-1] \
                * self.df_runtime[self.symbol_1][index-1] * -1 + self.df_runtime["bnh_rolling_net_open_symbol1"][index-1] \
                * symbol1_price
            bnh_day_gross_pnl_symbol1 = round(bnh_day_gross_pnl_symbol1, 2)
            bnh_day_gross_pnl_symbol1_cum = bnh_day_gross_pnl_symbol1 + \
                self.df_runtime["bnh_day_gross_pnl_symbol1_cum"][index-1]

            # leg2 calculation
            diff_2 = stock_2_demat_req - \
                self.df_runtime["stock_2_demat_req"][index-1]
            bnh_buy_symbol2_qty = diff_2 if diff_2 > 0 else 0
            bnh_sell_symbol2_qty = abs(diff_2) if diff_2 < 0 else 0
            bnh_net_open_symbol2 = bnh_buy_symbol2_qty - bnh_sell_symbol2_qty
            bnh_rolling_net_open_symbol2 = bnh_net_open_symbol2 + \
                self.df_runtime["bnh_rolling_net_open_symbol2"][index-1]
            bnh_day_gross_pnl_symbol2 = bnh_sell_symbol2_qty * symbol2_price - bnh_buy_symbol2_qty * symbol2_price + \
                bnh_net_open_symbol2 * symbol2_price + self.df_runtime["bnh_rolling_net_open_symbol2"][index-1] \
                * self.df_runtime[self.symbol_2][index-1] * -1 + self.df_runtime["bnh_rolling_net_open_symbol2"][index-1] \
                * symbol2_price
            bnh_day_gross_pnl_symbol2 = round(bnh_day_gross_pnl_symbol2, 2)
            bnh_day_gross_pnl_symbol2_cum = bnh_day_gross_pnl_symbol2 + \
                self.df_runtime["bnh_day_gross_pnl_symbol2_cum"][index-1]

        bnh_day_total_gross_pnl = bnh_day_gross_pnl_symbol1 + bnh_day_gross_pnl_symbol2
        bnh_day_total_gross_pnl_cum = bnh_day_gross_pnl_symbol1_cum + \
            bnh_day_gross_pnl_symbol2_cum
        bnh_plus_strategy_gross_pnl_cum = round(
            strategy_net_cum_pnl + bnh_day_total_gross_pnl_cum, 2)

        # appending into df
        # Note- renamed "cum_strategy_day_gross_pnl" to "strategy_gross_cum_pnl"
        record = {"Date": date,
        ...
        self.df_runtime = self.df_runtime.append([record], ignore_index=True)

    def build_table(self):
        self.df = self.df.reset_index()
        for index, row in self.df.iterrows():
            self.calculate(row['Date'], row[self.symbol_1],
                           row[self.symbol_2], index)
